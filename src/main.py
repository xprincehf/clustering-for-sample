...
if __name__ == '__main__':
    ...
    for i in range(iter_time):
        ...
        cluster = cluster.base_cluster(config, attribute_values)
    ...
    average_time = round(sum([x[1] for x in estimate_iter]) / len(iter_time), 2)
    average_silhouette_score = round(sum([x[2] for x in estimate_iter]) / len(iter_time), 4)
    average_calinski_harabaz_score = round(sum([x[3] for x in estimate_iter]) / len(iter_time), 4)

    estimate_iter.append([average_time, average_silhouette_score, average_calinski_harabaz_score])
